...
amp::MultiAgentPath2D MyDecentralPlanner::plan(const amp::MultiAgentProblem2D& problem) {
    ...
    for (size_t bot_num = 0; bot_num < num_bots; bot_num++){
        ...
        amp::CircularAgentProperties property = problem.agent_properties[bot_num];
        ...
        for (int i = 0; i < all_paths.size(); i++){
            MyObstacle my_ob;
            amp::CircularAgentProperties robo_property = problem.agent_properties[i];
            my_ob.defineWithDiscRadius(robo_property.radius);
            my_disc_obstacles.push_back(my_ob);
        }
        // gotta define this robo as an ob
        MyObstacle my_ob;
        amp::CircularAgentProperties robo_property = problem.agent_properties[bot_num];
        my_ob.defineWithDiscRadius(robo_property.radius);
        my_disc_obstacles.push_back(my_ob);
