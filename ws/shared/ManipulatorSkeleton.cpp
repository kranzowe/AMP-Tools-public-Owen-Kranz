...
amp::ManipulatorState MyManipulator2D::getConfigurationFromIK(const Eigen::Vector2d& end_effector_location) const {
    ...
    if (nLinks() == 3) {
        for (double theta_1 = 0; theta_1 <= 2.0*M_PI + 0.1; theta_1 += 0.1){
            ...
            double test_cos_theta_2;

            double xt = end_effector_location_copy[0];
            double yt = end_effector_location_copy[1];
            test_cos_theta_2 = (1.0/(2.0*a1*a2))*((xt*xt + yt*yt) - (a1*a1 + a2*a2));
            
            if (std::abs(test_cos_theta_2) <= 1.0) {
                // solution found!
        ...
        amp::ManipulatorState njoint_angles = nJointLogic(end_effector_location);
        return njoint_angles;
    }
...
amp::ManipulatorState MyManipulator2D::nJointLogic(const Eigen::Vector2d& end_effector_location) const {
    ...
    // okay so, we need to basically turn this into a 2link problem
    // gonna do that by turning all the links prior to the last two into
    // one link. 
    // this is accomplished by either setting their angle to 0 or 180

    double max_len_last_two = m_link_lengths[-1] + m_link_lengths[-2];

    double distance_to_go = end_effector_location.norm();

    int index_of_end = 0;

    while (distance_to_go > max_len_last_two) {
